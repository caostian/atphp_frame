...
class SessionUtil
{
    ...
    public static function init(array $config = [])
    {
        ...
        // 记录初始化信息
       DEBUG && LogUtil::record('[ SESSION ] INIT ' . var_export($config, true), 'info');
        $isDoStart = false;
        if (isset($config['use_trans_sid'])) {
            ini_set('session.use_trans_sid', $config['use_trans_sid'] ? 1 : 0); //是否用Query_String 来传递session_id
        }
        ...
        if (!empty($config['type'])) {
            ...
            if (!class_exists($class) || !session_set_save_handler(new $class($config))) {
                LogUtil::write("error session handler:' . $class");
                if(DEBUG){
